...
const AnimatedOptionsPanelOuter = styled(OptionsPanelOuter)`
  @media (min-width: ${LAYOUT_PIVOT}px) {
    min-height: 550px;
  }
`

const AnimatedPreviewOuter = styled(PreviewOuter)`
  @media (min-width: ${LAYOUT_PIVOT}px) {
    min-height: 550px;
  }
`
...
/* ─── Preview Panel (video-based, differs from shared image PreviewDisplay) ─ */
    ...
    >
      ...
      {historyReady && (
        <ScreenshotHistory
          ...
          getThumbnailSrc={entry => entry.thumbnail || entry.previewImage}
        />
      )}
    </Container>
  )
...
const AnimatedWebsiteScreenshotPage = () => (
  <Layout>
    ...
    />
    <ApiDocsCard
      title='Animated Screenshot API documentation'
      description='Integrate animated screen capture into any app. Explore the full API reference with interactive examples, SDKs for every language, and copy-paste code snippets.'
    />
    <ProductInformation />
  </Layout>
)
